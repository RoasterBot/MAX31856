...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        Note that the MAX31856 Dual has two different voltages as per
        Peter.

        DUAL MAX31865       ARDUIO UNO REV 3      Photon
        ---------------------------------------------------
        V+      ---->       5V           ---->    3.3V
        GND     ---->       GND          ---->    GND
        SD0     ---->       D6~          ---->    D6
...
/** Looks like this will need to change for J-type thermocouples **/
...
void setup() {
  ...
  temperature = new MAX31856(SDI, SDO, CS, CS2, SCK);
  
  // Initializing the MAX31855's registers
  temperature->writeRegister(REGISTER_CR0, CR0_INIT, CS, CS2);
  temperature->writeRegister(REGISTER_CR1, CR1_INIT, CS, CS2);
  temperature->writeRegister(REGISTER_MASK, MASK_INIT, CS, CS2);
...
void loop () {
  ...
  t = temperature->readJunction(FAHRENHEIT,CS);
  Serial.print("Junction (IC) temperature =");
  printTemperature(t);
  
  // Display the thermocouple temperature
  t = temperature->readThermocouple(FAHRENHEIT, CS);
  Serial.print("  Thermocouple1 temperature = ");
  printTemperature(t);

  // Display the thermocouple temperature
  t2 = temperature->readThermocouple(FAHRENHEIT, CS2);
  Serial.print("  Thermocouple2 temperature = ");
  printTemperature(t2);
